...
startofloop:
  ...
  uint16_t tx_batt = round(100.0*volt);
  uint16_t tx_runtime = (after-before2 ) & 0xFF;
  uint16_t tx_depth = round(10.0*dpt);
  
  uint8_t tx_arr[] = {0x00, MODULE_ID, 0x01, msb(tx_temp), lsb(tx_temp), 0x04, msb(tx_depth), lsb(tx_depth), 0x05, msb(tx_batt), lsb(tx_batt), 0x0D, msb(tx_runtime), lsb(tx_runtime), 0xFF};
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    if(  radio.write(&tx_arr, sizeof(tx_arr))  )
    {
      #ifdef S_VERB
      Serial.print("Sent: ");
      for(int i = 0; i < (sizeof(tx_arr)/sizeof(tx_arr[0])); i++)
      {
        Serial.print(tx_arr[i], HEX);
        Serial.print(", ");
      }
      Serial.println("");
      #endif
  ...
  after = millis();
  #ifdef S_VERB
  Serial.print("Switch with timestep: ");
  Serial.println(timestep);
  #endif
  if(timestep == T20TO6)
  {
    #ifdef S_VERB
    Serial.println("20TO6");
    #endif
    uint8_t tx_arr[] = {0x00, MODULE_ID, 0x0E, 0xFF};
    radio.stopListening();
    radio.write(&tx_arr, sizeof(tx_arr));
    radio.startListening();
    reResp = 0;
    while(!radio.available() && reResp < 500)
    {
      #ifdef S_VERB
    ...
    if(radio.available())
    {
      ...
      radio.stopListening();
      if(rcv_arr[0] == 0x00 && rcv_arr[1] == MODULE_ID && rcv_arr[2] == 0x0E)
      {
        uint16_t remain = ((rcv_arr[3] << 8) | rcv_arr[4]);
        if(remain > 0 && remain < 10000)
        {
          #ifdef S_VERB
          Serial.print("Got new remain: ");
  ...
  out += after-before2; //1
  out += ";";
  out += round(100*volt); //2
  out += ";";
  out += round(10.0*bme.readTemperature()); //3
  out += ";";
  out += round(10.0*bme.readHumidity()); //4
  out += ";";
  out += round(10.0*dpt); //5
  out += " ";
  //Serial.println(out);
